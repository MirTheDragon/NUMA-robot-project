...
bool showBootPrompt(SDL_Joystick*& joystick) {
    ...
    // 2) Create a fullscreen window & renderer
    SDL_DisplayMode dm;
    ...
    // 3) Load a font at size scaled for fullscreen
    int fontSize = dm.h / 20;  // ~5% of screen height
    ...
    // 4) Render the prompt into a texture
    const char* msg = "Press Y to boot NUMA hexapod\nPress X to exit";
    SDL_Color white = {255,255,255,255};
    // wrap at 80% of screen width
    ...
    while (!done) {
        ...
        while (SDL_PollEvent(&e)) {
            ...
            if (e.type == SDL_JOYBUTTONDOWN) {
                // replace these indices with your controller's mapping
            ...
            if (e.type == SDL_KEYDOWN) {
                if (e.key.keysym.sym == SDLK_y) { boot = true;  done = true; }
                if (e.key.keysym.sym == SDLK_x) { boot = false; done = true; }
            }
        }
    ...
    TTF_Quit();
    // leave SDL video subsystem up for next stages
